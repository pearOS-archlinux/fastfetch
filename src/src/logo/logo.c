...
// Extracts distro name from PRETTY_NAME (e.g. "PearOS 16" -> "PearOS", "Ubuntu 24.04 LTS" -> "Ubuntu")
static void logoPrettyNameToDistroName(const FFstrbuf* prettyName, FFstrbuf* out)
{
    ffStrbufClear(out);
    if (prettyName->length == 0) return;

    uint32_t end = prettyName->length;
    for (uint32_t i = 0; i < prettyName->length; i++)
    {
        char c = prettyName->chars[i];
        if (c == '(' || (c >= '0' && c <= '9'))
        {
            end = i;
            break;
    ...
    ffStrbufSetNS(out, end, prettyName->chars);
    ffStrbufTrimRightSpace(out);
}
...
static const FFlogo* logoGetBuiltinDetected(FFLogoSize size)
{
    ...
    const FFlogo* logo = logoGetBuiltin(&os->prettyName, size);
    if(logo != NULL)
        return logo;

    FF_STRBUF_AUTO_DESTROY prettyNameStripped = ffStrbufCreate();
    logoPrettyNameToDistroName(&os->prettyName, &prettyNameStripped);
    if(prettyNameStripped.length > 0)
    {
        logo = logoGetBuiltin(&prettyNameStripped, size);
        if(logo != NULL)
            return logo;
    }
